...
const defaultDelayStrategy = (attempt: number, baseDelayMs: number, maxDelayMs: number): number => {
...
const serializeError = (err: unknown): string => (err instanceof Error ? err.message : String(err));
...
export const withRetry = async <T>(
  ...
  const {
    ...
    delayStrategy = (attempt) => defaultDelayStrategy(attempt, baseDelayMs, maxDelayMs),
    signal,
    onMetrics,
  } = options;
